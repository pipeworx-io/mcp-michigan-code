...
/**
 * Michigan Compiled Laws — state statutes by citation.
 *
 *   MCL 750.316  ->  /Laws/MCL?objectName=mcl-750-316
 *
 * The dot in the citation becomes a dash in the object name, which is the only
 * transformation involved. A nonexistent section returns HTTP 400 — an honest
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Michigan Compiled Laws');
}

const BASE = 'https://www.legislature.mi.gov/Laws/MCL?objectName=mcl-';
const UA = 'pipeworx-mcp-michigan-code/1.0 (+https://pipeworx.io)';

const tools: McpToolExport['tools'] = [{
  name: 'mi_compiled_law',
  description:
  ...
  inputSchema: {
    type: 'object',
    properties: {
      section: { type: 'string', description: 'Section as cited, e.g. "750.316" or "257.625". Chapter is the part before the dot.' },
    },
    required: ['section'],
  },
}];
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  if (name !== 'mi_compiled_law') throw new Error(`Unknown tool: ${name}`);
  const raw = String(args.section ?? '').trim().replace(/\s+/g, '').replace(/^MCL/i, '');
  if (!/^\d+[a-z]?\.\d+[a-z]?$/i.test(raw)) {
    return { found: false, reason: 'unparsed_section',
      message: `"${args.section}" is not a Michigan Compiled Laws citation.`,
      hint: 'MCL citations look like "750.316" (murder 1) or "257.625" (OWI).' };
  }
  ...
  if (res.status === 400 || res.status === 404) {
    return { found: false, reason: 'section_not_found', section: raw,
      message: `No section ${raw} in the Michigan Compiled Laws (HTTP ${res.status}).`,
      hint: 'Section numbers are not contiguous — repealed sections are normal.' };
  }
  if (!res.ok) throw new Error(`Michigan Legislature returned HTTP ${res.status} for ${raw}`);
  const full = statuteText(await res.text());

  // Michigan renders the whole site nav into the document, so ~2,000
  // characters of "Skip to content / Bills / Resolutions" precede the law.
  // Anchor at the line opening with the section number, the same fix the other
  // state packs needed. Verified against the LIVE gateway rather than locally:
  // legislature.mi.gov serves an incomplete TLS chain that Node's fetch
  // rejects outright, so this pack could only ever be tested through Workers.
  const lines = full.split('\n');
  const at = lines.findIndex((l) => l.trim().startsWith(raw));
  const text = at === -1 ? full : lines.slice(at).join('\n').trim();
  const heading = at === -1 ? null : (lines[at] ?? '').replace(raw, '').trim() || null;
  return {
    citation: `MCL § ${raw}`, section: raw, heading,
    text, text_chars: text.length, url,
    source: 'Michigan Legislature (legislature.mi.gov), keyless',
    attribution: 'Michigan statutes are public record.',
  };
}
